...
main() {
  ...
  # Cluster1 without CNI (cilium)
  # minikube start --bootstrapper=kubeadm --memory 8192 --cpus 4 --profile cluster1 \
  minikube start --bootstrapper=kubeadm --memory 2048 --cpus 2 --profile cluster1 \
  ...
  minikube start --bootstrapper=kubeadm --memory 2048 --cpus 2 --profile cluster2 \
                --extra-config=apiserver.authorization-mode=Node,RBAC \
                --extra-config=apiserver.service-account-signing-key-file=/var/lib/minikube/certs/sa.key \
                --extra-config=apiserver.service-account-key-file=/var/lib/minikube/certs/sa.pub \
                --extra-config=apiserver.service-account-issuer=api \
